...
public class RhWorkorderWindow extends MainWindow
implements InfWindow
{
	...
	throws Exception
	{
		...
		String wonum = "RH" + this.genAutokey("wonum");
		...
		newobj.setWorktype("RH");
		newobj.setEqdown("否");
		newobj.setHaschildren("否");
		newobj.setHasfollowupwork("否");
		newobj.setEstdur(0L);
		newobj.setReportedby(labornum);
		mainObject = newobj;
		return true;
	}
	...
	public void save() throws Exception {
		...
		List laborlist = this.getMainSrv().findWithQuery(Labor.class,"labornum ='" + enterbylabornum+ "'");
		...
		List ibsuserlist =this.mainSrv.getBaseDao().findWithQuery(Ibsgroupusers.class, "userid  = '"+ userid +"'");
		String grpname = null;
		String  sitenum = labor.getSitenum();
		...
		int allsave = (int)this.getMainSrv().getBaseDao().selectCountByHql("select count(*) from Labor t where t.labornum = '"+modifylabornum+"' and t.sitenum ='"+ sitenum +"'");
	 ...
	 * @author:yuanjq
	 * @throws Exception 
	 * @description:删除标准作业计划及相关信息
	 * @ 2007-7-6 上午10:21:06
	 */
	public void deljbplan() throws Exception
	{
		int rtn = Messagebox.show("是否确定删除当前工单的标准作业计划及所有关联的子记录吗？","删除确认！",Messagebox.YES | Messagebox.NO,Messagebox.QUESTION);
	...
	public void selectinvr() throws Exception
	{
		...
		this.popupDialog(this.getMainObject(), "/inventory/invreservepopup.xul", "wonum='"+wo.getWonum()+"'");
