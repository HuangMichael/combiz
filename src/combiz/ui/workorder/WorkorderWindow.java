...
public class WorkorderWindow extends MainWindow implements InfWindow {
	 ...
	 * 方法
	 * 
	 * 作者：李阳 功能：将工单里的标准作业计划添加到标准作业计划模板里。 日期：2008-4-15 下午04:58:45
	 * 
	 */
	 ...
	 * 方法
	 * 
	 * 作者：雷涛 修改：增加了是否复制工单的确认 日期：2008-5-12 下午12:23:45
	 * 
	 */
	public void copywo() throws Exception {
		int rtn = Messagebox.show("是否确定复制工单？", "确认是否复制！", Messagebox.YES
		...
		if (rtn == Messagebox.NO)
			...
			Messagebox.show("已成功将工单:" + wo.getWonum() + "复制到工单"
					+ newWO.getWonum());
			this.refreshData();
		}
	}
	...
	public void createinvr() throws Exception {
		int rtn = Messagebox.show("是否确定生成预留信息？", "生成！", Messagebox.YES
				| Messagebox.NO, Messagebox.QUESTION);
		if (rtn == Messagebox.NO)
			return;
		else if (rtn == Messagebox.YES) {
			...
			List wpmateriallist = this.getMainSrv().getBaseDao().findWithQuery(
					Wpmaterial.class, "wonum = '" + wonum + "'");
			List wptoollist = this.getMainSrv().getBaseDao().findWithQuery(
					Wptool.class, "wonum='" + wonum + "'");
			...
			List toolreservelist = this.getMainSrv().getBaseDao()
					.findWithQuery(Toolreserve.class,
							"reqnum = '" + wonum + "'");
	...
	public void initData() throws Exception {
		...
		ListWindow materiallistwnd = (ListWindow) this
				.getFellow("wpmaterialTable");
		...
		ListWindow hazpreclistwnd = (ListWindow) this
				.getFellow("wohazprecTable");
		ListWindow isotaglistwnd = (ListWindow) this.getFellow("woisotagTable");
		ListWindow isolockoutlistwnd = (ListWindow) this
				.getFellow("woisolockoutTable");
		ListWindow failclasslistwnd = (ListWindow) this
				.getFellow("wofailclass");
		ListWindow failcodelistwnd = (ListWindow) this.getFellow("wofailcode");
		ListWindow failproblemlistwnd = (ListWindow) this
				.getFellow("wofailproblem");
		ListWindow failcauselistwnd = (ListWindow) this
				.getFellow("wofailcause");
		...
		// ListWindow kpichartlistwnd =(ListWindow)
		// this.getFellow("kpichartwnd");
	...
	/**
	 * 功能：登记设备停机时间 作者：陈明锐 日期：2009-2-5
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * 功能：查看故障历史 作者：陈明锐 日期：2009-2-16
	 * 
	 * @throws Exception
	 */
	public void failhistory() throws Exception {
		if (this.selectedTabid.equals("list")) {
			Messagebox.show("请选择一条主记录!");
			return;
		}
		...
		this.popupDialog(this.mainObject, "/workorder/failhistorydialog.xul");
	}
	 ...
	 * @throws Exception
	 */
	public void wocostcount() throws Exception {
		if (this.selectedTabid.equals("list")) {
			Messagebox.show("请选择一条主记录!");
			return;
		}
		...
		this.popupDialog(workorder, "/workorder/countwocost.xul");
	}
	/**
	 * 根据物资编码添加BOM备件信息
	 * 
