...
public class FindClassificationPmTree extends LookupTree
{   
	private String type;
	ClassificationSrv classificationSrv;
	
	public FindClassificationPmTree() 
	{
		super();
	}
	...
	throws Exception
	{
		 ...
		 */
		String whereStr = "parent is null and classtype='预防性维护'";
	...
	{
		this.getChildren().clear();
		List list = classificationSrv.findWithQuery("classid='"+value+"' and classtype='预防性维护'");
		if(list.size()<=0)
		{
			Treechildren tc = new Treechildren();
	...
	throws Exception
	{
		List parentList = classificationSrv.findWithQuery("classid = '" + classification.getParent() + "' and classtype='预防性维护'");
	...
	throws Exception
	{
		if(parentitem!=null && parentitem.getValue() != null) //如果没有父级
		{
			 ...
			 * 如果是中间树，比如人员选择的时候的部门树，那么在过滤条件上必须加上this.getTreeQueryString("树的主表名-大写")
			 */
			String whereStr = "parent = '"+parentClass.getClassid()+"' and classtype='预防性维护'";
			if(this.getQueryString()!=null)
				whereStr = whereStr + " and " + this.getQueryString();
			List list = classificationSrv.findWithQuery(whereStr,"orderby");
			Treeitem ti;
