...
public class Fldcurbal extends FieldAdapter {
	...
	public Object initValue() throws Exception {
		...
		Eqsparepart eqsparepart = (Eqsparepart) this.getMainObject();
		sumcurbal = (Double) this.getMainSrv().getBaseDao().selectSumByHql("select sum(t.curbal) from Invstock t where  t.itemnum = '"+eqsparepart.getItemnum()+"' ");
		if(sumcurbal == null)
			sumcurbal = 0d;
		return sumcurbal;
	}
