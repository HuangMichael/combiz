...
public class Eqinvrecline extends CommonListWindow {
	...
	public void onCreate() throws Exception
	{
		...
		String abc = idstr2+"' and reqnum = '"+ matreqnum +"'and reqnum in (select t.reqnum from Invreserve t where (t.reservedqty - isnull((select sum(i.quantity) from Invusetrans i where i.state ='待确认' and i.itemnum = t.itemnum and i.matreqnum = t.reqnum),0)) >0 and (t.reqnum is not null and DATALENGTH(t.reqnum) <> 0))";
		List InvrlineList;
		if(IBSServer.getIBSServer().getDatabaseProductName().equals("SQLSERVER"))
		{
			InvrlineList = this.getMainSrv().getBaseDao().findWithQuery(Invreserve.class, idstr2+" and reqnum = '"+ matreqnum +"'and reqnum in (select t.reqnum from Invreserve t where (t.reservedqty - isnull((select sum(i.quantity) from Invusetrans i where i.state ='待确认' and i.itemnum = t.itemnum and i.matreqnum = t.reqnum),0)) >0 and (t.reqnum is not null and DATALENGTH(t.reqnum) <> 0))");
		}
		else
		{
			InvrlineList = this.getMainSrv().getBaseDao().findWithQuery(Invreserve.class, idstr2+" and reqnum = '"+ matreqnum +"'and reqnum in (select t.reqnum from Invreserve t where (t.reservedqty - nvl((select sum(i.quantity) from Invusetrans i where i.state ='待确认' and i.itemnum = t.itemnum and i.matreqnum = t.reqnum),0)) >0 and t.reqnum is not null)");
		}
