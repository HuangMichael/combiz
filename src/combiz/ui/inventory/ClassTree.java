...
import combiz.ui.basedata.ClassificationWindow;
...
public class ClassTree extends MainTree
{
	...
	throws Exception
	{
		...
		mainWnd = (ClassificationWindow)this.getFellow("mainWnd");
		classificationSrv = (ClassificationSrv)IBOSrvUtil.getSrv("classification");
		classificationSrv.setOrderBy("orderby,classid");
		//List list = classificationSrv.findWithQuery("classtype='物资' and parent is null");
		List list = classificationSrv.findWithQuery(" classtype in ('物资','资产','工具') and parent is null");
		...
		for(int i=0;i<list.size();i++)
		{
			...
			if(desc==null) 
				desc = "";
			ti = new Treeitem(desc);  //+":"+lochiery.getLocation()
	...
	throws Exception
	{
		if(parentitem!=null && parentitem.getValue() != null) //如果没有父级
		{
			...
			//List list = classificationSrv.findWithQuery("classtype in('物资') and parent = '"+lochparent.getClassid()+"'");
			classificationSrv.setOrderBy("orderby,classid");
			List list = classificationSrv.findWithQuery(" classtype in('物资','资产','工具') and parent = '"+lochparent.getClassid()+"'");
			Treeitem ti;
			for(int i=0;i<list.size();i++)
			{
				Classification classification = (Classification)list.get(i);
	...
	throws Exception
	{
		...
		item.setLabel(classification.getDescription()); //+":"+classification.getClassid()
